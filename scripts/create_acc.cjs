...
async function main() {
    ...
    try {
        ...
        await page.goto('https://account.hackclub.com', { waitUntil: 'networkidle2' });
        ...
        await page.waitForFunction('!!document.querySelector("div.p-client_container")', { timeout: 30000 });
        await sleep(1000);
        ...
        await page.waitForFunction('!!document.querySelector("div.p-top_nav__search__container")', { timeout: 30000 });
        ...
        try {
            ...
            if (cocBtn) {
                await cocBtn.click();
                await page.waitForFunction('!!document.querySelector(\'[data-qa-action-id="tutorial_agree"]\')', { timeout: 10000 });
        ...
        try {
            ...
            await page.goto('https://api.slack.com/apps', { waitUntil: 'networkidle2' });
            await sleep(2000);
            ...
            await page.click('[data-qa="create_app_button"]').catch(async () => {
                await page.evaluate(() => {
                    ...
                    const createBtn = buttons.find((b) => b.textContent.includes('Create New App') || b.textContent.includes('Create an App'));
            ...
            const tabsContainer = await page.$('[data-qa="tabs_content_container"] textarea, [data-qa="tabs_content_container"] [contenteditable="true"]');
            if (tabsContainer) {
            ...
            if (oauthPageExists) {
                ...
                const clicked = await page.evaluate(() => {
                    ...
                    const oauthLink = links.find((l) => l.textContent.includes('OAuth') || l.href?.includes('oauth'));
                    if (oauthLink) {
            ...
            const installHref = await page.evaluate(() => {
                ...
                const span = Array.from(document.querySelectorAll('a.c-button--primary span.content')).find(
                    (s) => s.textContent.includes('Install to Hack Club')
                );
                if (span) {
                ...
                const links = Array.from(document.querySelectorAll('a'));
                const installLink = links.find((a) => a.textContent.includes('Install to Hack Club'));
                if (installLink) {
            ...
            console.error('Error during app creation (browser kept open):', appError.message);
        ...
        console.log('xoxd:', xoxd);
        console.log('\nTo sign in manually:');
        console.log(`document.cookie = "d=${xoxd}; Domain=.slack.com"; location.href = "https://hackclub.slack.com"`);
        ...
        console.error('Error during account creation (browser kept open):', error.message);
        console.log('Browser kept open for manual intervention. Press Ctrl+C to exit.');
        await new Promise(() => { }); // Wait indefinitely
    }
}
